...
config_docker(){
    ...
    echo "等待加载DOCKER配置文件"
    curl -L https://raw.githubusercontent.com/520qdd/v2raypj/master/docker-compose.yml > docker-compose.yml
...
install_docker(){
    ...
    echo "4、玄学问题 你太帅导致的 指导联系TG@hxslk"
    echo
}
...
install_select(){
    ...
    case "${selected}" in
        ...
        ;;
        *)
        echo -e "[${red}Error${plain}] 输入错误,请输入正确数字"
        ;;
    esac
    done
}
...
while true
do
echo -e "\033[42;30m 脚本 By：靓坤 \033[0m"
echo -e "\033[42;30m 商用请购买正版 靓坤TG:@hxslk \033[0m"
echo  ""
echo  "请输入数字选择你要进行的操作："
...
case "${selected}" in
    ...
    ;;
    *)
    echo -e "[${red}Error${plain}] 输入错误,请输入正确数字 [1-4]"
    ;;
esac
done
